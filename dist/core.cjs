...
/**
   ...
   * @returns {void}
   * @throws {APIError} - Throws an error if the user is not authenticated.
   */async linkTikTok(t){if(!this.isAuthenticated)throw new s("User needs to be authenticated");const e=await fetch(`${u}/tiktok/connect`,{method:"POST",redirect:"follow",headers:{Authorization:`Bearer ${this.jwt}`,"x-client-id":this.clientId,"Content-Type":"application/json"},body:JSON.stringify({userHandle:t,clientId:this.clientId,userId:this.userId})}).then((t=>t.json()));if(e.isError)throw new s(e.message||"Failed to link TikTok account");return e.data}async linkTelegram(t){if(!this.isAuthenticated)throw new s("User needs to be authenticated")}
/**
